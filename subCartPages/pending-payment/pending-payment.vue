		...
		<view class="price-detail">
			<view class="price-detail-desc">
				...
				<view class="price-detail-button">
					<u-button color="red" size="small" shape="circle" @click="continuePayment" text="继续付款"></u-button>
				</view>
			</view>
		</view>
		<view class="">
...
<script>
	...
	import { pendingPaymentContinue,getUserBalanceNumber,getPendingPaymentOrderById } from '@/api/pay/pay.js'
	import {
		mapState,
		mapActions,
		mapGetters
	...
	export default {
		...
		async onLoad(o) {
			...
			const {data:res } = await getPendingPaymentOrderById(this.orderId)
			if(res.code ==0 ){
		...
		methods: {
			...
			async continuePayment(){
				...
				if(this.finalPrice > balanceRes.data){
					...
					// 扣款
					const { data:res } = await pendingPaymentContinue(this.pendingPaymentOrder)
					if(res.code == 0){
						uni.showToast({
							title: "下单成功",
							icon: 'success',
						...
						setTimeout(()=>{
							uni.redirectTo({
								url: 'subCartPages/checkout-success/checkout-success?orderId=' + this.orderId
							})
						},2000)
					}
				}
			...
			async createOrder(i){
				...
				this.order.commodityList.map(o => {
					...
					order.orderRemarks = o.orderRemarks != "" || o.orderRemarks != null ? o.orderRemarks : ""
