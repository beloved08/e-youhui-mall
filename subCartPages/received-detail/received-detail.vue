		...
		<view class="price-detail">
			<view class="price-detail-desc">
				<view class="price-detail-item">
					...
					<u-button size="small" type="primary" shape="circle" @click="confirmReceipt" text="确认收货"></u-button>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		async onLoad(o) {
			...
			const { data: orderDetailRes } = await getOrderDetailByNumber(o.orderNumber)
			const { data: res } = await getExpressOrderDetail(o.orderNumber)
		...
		methods: {
			async confirmReceipt(){
				const { data:res } = await userConfirmReceipt(this.expressOrderId,this.orderNumber)
				if(res.code == 0){
					uni.showToast({
						title: '收货签收成功',
						duration: 2000,
					...
					setTimeout(() => {
						uni.redirectTo({
							url: 'subPages/personal-pages/order/order?currentTabIndex=4'
						})
					},2000)
				}else{
					uni.showToast({
