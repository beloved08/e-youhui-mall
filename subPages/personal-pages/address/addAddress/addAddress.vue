		...
		<view class="shop-box">
			<view class="apply-shop">
				...
				<view class="apply-from">
					...
					<div class="apply-from-item apply-from-item-map">
					...
					<!-- <div class="apply-from-item">
						...
						<view class="" style="margin-top: 10rpx;">
							<radio-group @change="addressChange">
								<radio  value="0" style="transform:scale(0.7)" /><text style="font-size: 13px;">默认地址</text>
								<radio  value="1" style="transform:scale(0.7)" /><text style="font-size: 13px;">普通地址</text>
							</radio-group>
						</view>
					</div> -->
				</view>
			...
			<view class="button-box">
				<u-button type="primary" text="保存收货地址" @click="saveAddress"
...
<script>
	...
	import { userAddAddress } from '@/api/user/user.js'
	export default {
		computed: {
			...mapState(['user']),
		},
		...
		methods: {
			...
			async saveAddress(){
				if(this.userAddressForm.consignee == ''){
					...
					const { data: res } = await userAddAddress(this.userAddressForm)
					if(res.code == 0){
						uni.showToast({
							title: res.data,
							icon:'none'
						})
						this.userAddressForm.detailedAddress = ''
						setTimeout(()=>{
