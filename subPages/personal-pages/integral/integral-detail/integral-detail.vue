	...
	<view class="container">
		<view class="change-list">
			<tui-list-view color="#777">
				<tui-list-cell hover :arrow="false" v-for="(item,index) in allUserIntegralChangeDetailList" :key="index">
...
<script>
	import { getUserIntegralChangeDetail } from '@/api/pay/pay.js'
	export default {
		data() {
			return {
		...
		methods: {
			// 获取变动明细
			async selectAllUserIntegralChangeDetail(){
				const { data:res } = await getUserIntegralChangeDetail(this.userId,1)
				this.allUserIntegralChangeDetailList = res.data
			}
		}
	}
</script>
